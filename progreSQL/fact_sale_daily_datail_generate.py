...
def create_tables(cursor):
    ...
    order_types = """
        ...
        'normal', 'flash', 'luckyDraw', 'mixed', 'redeem', 'promotion');
        """
    ...
    fact_sales_daily_detail = """CREATE TABLE fact_sales_daily_detail (
                    ...
                    coins_redeem MONEY,
                    voucher_redeem MONEY,
                    success_voucher_redeem MONEY,
                    success_coin_redeem MONEY,
                    item_discount MONEY,
                    success_item_discount MONEY,
                    postage_discount MONEY,
                    success_postage_discount MONEY,
                    confirm_gmv MONEY,
                    reject_gmv MONEY,
                    cancel_gmv MONEY,
                    online_paid MONEY,
    ...
    for it in seller_db.Store.find():
        cursor.execute(
            sql.SQL("insert into {} values (%s, %s, %s, %s)").format(
                ...
                it["_id"], it["regionId"], it["sellerRegionId"], it["name"]])
        ...
        for it in order_db.SaleOrderDetail.find({"orderId": so["id"]}):
            ...
            logging.info(
                ...
                % (so["batchId"], so["billId"], it["orderId"], it["id"],
                   ...
                   listing["categoryId"], it["listingId"], it["skuId"],
                    ...
                    so["region"], so["warehouseId"],
                   warehouse_di[so["warehouseId"]],
            ...
            cursor.execute(
                sql.SQL("insert into {} values (%s, %s, %s, %s, %s, %s, %s, %s,"
                    ...
                    store["name"], address["areaId"], address["cityId"],
                    so["region"], so["warehouseId"],
                    warehouse_di[so["warehouseId"]], it["dealPrice"],
                    it["count"], it["amount"], postage_di[postage_index],
                    it.get("coin", 0), it.get("redeem", 0),
                    it.get("voucherRedeem", 0), it.get("discount", 0),
                    pos_dis_di[postage_index], order_total, need_pay, paid,
                    cod_amount, payment_type, order_time, order_tp, pay_time,
                    confirm_time, pay_during, confirm_during])
            postage_index += 1
