...
def shut_down_pf_address(area_ids, pf_areas):
    # cities = ["Dhaka (Sub-Area)", "Dhaka-South", "Savar", "Dhaka-North"]
    city_ids = common_db.city.distinct(
        "id", {"name": {"$in": shut_down_cities}, "regionId": 1})
    shut_down_update = {"$set": {"enabled": False, "visible": False}}
    res = common_db.city.update_many({"id": {"$in": city_ids}}, shut_down_update)
    print(res.matched_count, res.modified_count)
    res = common_db.area.update_many(
        {"cityId": {"$in": city_ids}}, shut_down_update)
    print(res.matched_count, res.modified_count)
    res = member_db.address.update_many(
        {"cityId": {"$in": city_ids}}, {"$set": {"deleted": True}})
    print(res.matched_count, res.modified_count)
    res = member_db.address.update_many(
        {"areaId": {"$nin": area_ids}}, {"$set": {"deleted": True}})
    print(res.matched_count, res.modified_count)
    res = common_db.area.update_many(
        {"id": {"$nin": area_ids}, "regionId": 1, "minAppVersion": {"$ne": 11}},
        shut_down_update)
    print(res.matched_count, res.modified_count)
    res = common_db.area.update_many(
        {"stateId": 1, "$or": shut_down_areas}, shut_down_update)
    print(res.matched_count, res.modified_count)
    res = common_db.area.update_many(
        {"name": {"$nin": pf_areas}, "regionId": 1,
         "minAppVersion": {"$ne": 11}},
        shut_down_update)
    print(res.matched_count, res.modified_count)
...
if __name__ == "__main__":
    ...
    shut_down_pf_address(area_ids, list(pf_areas))
