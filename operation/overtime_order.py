...
def get_online_order():
    ...
    for it in order_db.SaleOrder.find(query_1).sort([("_id", -1)]):
        if it.get("closedAt"):
            if (it["closedAt"] - it["createdAt"]).days >= 45:
                order_li.append(it)
        elif (time_now - it["createdAt"]).days >= 45:
            order_li.append(it)
    ...
    worksheet_1.write("I1", "Pay Method", bold)
    ...
    for item in order_li:
        ...
        worksheet_1.write('I%d' % row_1, pay_method_map[item["payMethod"]])
