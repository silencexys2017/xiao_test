...
FILE_NAME = "sold_sku_statistics.xlsx"
...
def export_sold_sku(year, start_month, end_month):
    # 客户ID/姓名/地址/电话
    order_ids = order_db.SaleOrder.distinct(
        "id", {"createdAt": {
            "$gte": datetime(year, start_month, 1),
            "$lt": datetime(year, end_month, 1)},
            "status": {"$nin": [1, -1]}})
    sku_di = {}
    listing_ids = set()
    for it in order_db.SaleOrderDetail.aggregate(
        [
            {"$match": {"orderId": {"$in": order_ids}}},
            {"$group": {"_id": {"skuId": "$skuId", "listingId": "$listingId"},
                        "count": {"$sum": "$count"}}},
            {"$sort": {"count": -1}}
        ]
    ):
        sku_di[it["_id"]["skuId"]] = {
            "listingId": it["_id"]["listingId"], "count": it["count"]}
        listing_ids.add(it["_id"]["listingId"])
    listing_di = {it["_id"]: it["storeId"] for it in goods_db.SpecOfListing.find(
        {"_id": {"$in": list(listing_ids)}}, {"storeId": 1})}
    worksheet = workbook.add_worksheet(
        "sold_sku_%s_(%s-%s)" % (year, start_month, end_month-1))
    headings = [
        'skuId', 'listingId', 'storeId', 'count'
    ]
    ...
    for k, v in sku_di.items():
        ...
        worksheet.write('B%d' % row, v.get("listingId"))
        worksheet.write('C%d' % row, listing_di.get(v.get("listingId")))
        worksheet.write('D%d' % row, v.get("count"))
...
if __name__ == "__main__":
    ...
    export_sold_sku(2021, 1, 4)
    export_sold_sku(2021, 4, 7)
    workbook.close()
