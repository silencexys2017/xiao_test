...
APP_CODE = "fdf8623e0f534be0bdd64dd191c472832b732482cdea434d9352d00538e40cde"
TOKEN = "testToken"
APP_ID = "2"
UPLOAD_URL = "https://metricscenter-api-dra.cloud.huawei.com:443/recommend" \
             "/datacollection/import/orderinfo"
...
def request_hw_order_info_api(data_opt_type, order_info):
    headers = {
    ...
    data = {
        "appid": APP_ID,
        "industry_type": 1,  # 行业类型: 1电商行业 2视频行业 3新闻行业
        "timestamp": int(round(time.time() * 1000)),
        "data_opt_type": data_opt_type,
        "orderdata": order_info
    }
    logging.info("request_hw_order_info request_data=%r" % data)
    res = requests.post(
        UPLOAD_URL, headers=headers, data=json.dumps(data))
    if res.status_code in [200, 201]:
        res_json = res.json()
        if res.json().get("result_code") in [200, "200"]:
            logging.info("request success: %s" % res_json)
            return "complete"
        else:
            logging.error("request failed: %s" % res.content)
            return "requeue"
    else:
        logging.error("request failed: %s" % res.content)
...
def get_related_data():
    ...
    #     {"id": {"$gt": 314782, "$lte": 314786}}).sort([("id", 1)])
    so_cursor = order_db.SaleOrder.find(
        {"id": {"$gt": 314786, "$lt": 315982}}).sort([("id", 1)])
    ...
    try:
        for so in so_cursor:
            ...
            request_hw_order_info_api(data_opt_type="1", order_info=[hw_obj])
